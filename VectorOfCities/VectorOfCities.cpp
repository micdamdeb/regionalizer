...
void calculateHappinessLevelForOneRegionByBruteForce(std::vector<City> vec)
{
    ...
    for (City i : vec) {
        ...
        for (City j : vec) {
            std::size_t tmp = (10.0 - ((static_cast<double>(j.distanceFrom(i)) / static_cast<double>(j.getDistanceToTheFarthest())) * 10.0));
    ...
    std::cout << std::endl;
    std::cout << "Percentage = " << static_cast<double>(10.0 * bestHLsoFar) / getPopulationOfTheWholeVector(vec) << "%";
    std::cout << std::endl;
}
    ...
    for (; it != sortedVec.begin() + 8; ++it) {
        for (jt = it + 1; jt != sortedVec.begin() + 9; ++jt) {
            ...
            for (const auto& i : sortedVec) {
                ...
                std::size_t distanceToTheClosestCapital = distanceToit > distanceTojt ? distanceTojt : distanceToit;
                std::size_t tmp = (10.0 - ((static_cast<double>(distanceToTheClosestCapital) / static_cast<double>(i.getDistanceToTheFarthest())) * 10.0));
